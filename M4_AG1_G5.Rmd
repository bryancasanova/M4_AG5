...
Como en nuestro escenario `k=4` tenemos que escoger 1 estado representante de cada cluster, para ello se verá la distnacia con respecto al centroide. Aquel estado más cercano al centroide será el escogido.
...
distance_to_center <- function(datos_scaled, k, km_clusters){
  ...
  for (row in 1:nrow(data_filter)){
        k_distance <- sqrt(sum(data_filter[row, -1] - center)^2)
  ...
  return (c(state, distance))
  
}


representantes <- c(distance_to_center(datos_scaled, 1, km_clusters)[1], distance_to_center(datos_scaled, 2, km_clusters)[1], distance_to_center(datos_scaled, 3, km_clusters)[1], distance_to_center(datos_scaled, 4, km_clusters)[1])

print(representantes)
```

Nota: El cluster 4 solo cuenta con un uno estado!.
...
```
El Cluster 4 posee un alto nivel de 


* **Ejercicio 8**: ¿Cuál de los estados seleccionados en el ejercicio anterior tiene una tasa de desempleo más elevada?

En primer lugar vamos a asignar la etiqueta de los clusters a los datos originales.

```{r}
datos$cluster <- km_clusters$cluster

t(aggregate(datos, by = list(datos$cluster), FUN = mean))['StateUnemployRate', ]


```
El cluster que tiene una mayor tasa de desempleo es el cluster 4 con un unico valor
...
t(aggregate(datos, by = list(datos$cluster), FUN = mean))[c("PercentExportSmallBiz", "PercentExportValue"), ]
```
El cluster 4 es el cluster con los indicadores apropiados para esa campaña, pero al ser de un unico estado, tambien se recomienda incorporar el cluster 3, debido a que tiene, notoriamente, un valor de exportación más pequeño.
...
```{r}
# Introduce aquí tu respuesta
t(aggregate(datos, by = list(datos$cluster), FUN = mean))[c("MedIncomeSelfEmpUnincorp"), ]

```
En general el ingreso es bastante parejo, el cluster 4 es el que tiene mayores ingresos, pero es solo 1 estado. Por lo que tambien se destaca el cluster 2 que esta tambien tiene un buen ingreso.
